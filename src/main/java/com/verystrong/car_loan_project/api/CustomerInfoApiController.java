...
public class CustomerInfoApiController {
    ...
    public ResponseEntity<Object> update(@PathVariable Long customerId, @RequestBody CustomerInfoDto dto)
    {
        //1.DTO->엔티티 변환하기
        CustomerInfo customerInfo = dto.toEntity();
        log.info("id:{}, customerInfo:{}", customerId, customerInfo.toString());
        // 2. id 찾기
        CustomerInfo target = customerInfoRepository.findById(customerId).orElse(null);
        // 3. 잘못된 요청 처리하기
        if (target == null || customerId!=target.getCustomerId()){
            log.info("잘못된 요청 id {} customerInfo{} ", customerId, customerInfo.toString());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
        }
        //4. 업데이트 및 정상 응답(200)하기
        target.equals(customerInfo);
        CustomerInfo updated = customerInfoRepository.save(target);
        log.info("update OK {}", ResponseEntity.status(HttpStatus.OK).body(updated));
